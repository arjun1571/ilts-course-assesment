...
const Button: FC<ButtonProps> = ({
  ...
  const variants: Record<string, string> = {
    ...
    outline: "bg-white border border-primary-500 text-primary-500 custom-btn-shadow",
  ...
  const sizeStyles: string = size ? sizes[size] : `${sizes["sm"]} md:${sizes["base"]}`;
  const roundStyles: string = rounds[round];
  const variantStyles: string = variants[variant];
  const disabledStyles: string = disabled ? "!bg-disable-500 !bg-opacity-60 !text-white !cursor-not-allowed" : "";
